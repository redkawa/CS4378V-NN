...
def gather_sample_statistics():

    start = time.time()

    neural_net = MultilayerNeuralNetwork(9,  # Number inputs
                                         4,  # Number hidden nodes
                                         2,  # Number outputs
                                         48,  # Of iterations
                                         0.06202604050651971,  # Learning Rate
                                         0,  # Momentum
                                         0.01)  # Rate of Decay for Learning Rate
            ...
            for matrix_row in range(3):
                ...
                mean += (row[0][(270 + 300 * matrix_col) + (10 * matrix_row)] + row[0][
                    ...
                    (272 + 300 * matrix_col) + (10 * matrix_row)] + row[0][
                             ...
                             (279 + 300 * matrix_col) + (10 * matrix_row)])
                mean = mean / 100
                modified_sample_data.append(mean)
        row[0] = modified_sample_data
        modified_sample_data = []
    ...
    neural_net.demo(sample_data)  # test it on the test data
    end = time.time()

    print("Time it took to compute Optimized 9-input NN: " + str(end - start) + " seconds.")


def gather_statistics():

    print("Number of NN's to be tested: " + str(len(hidden_node_dict) * len(iterations_dict) * len(momentum_dict) * len(
    learning_rate_dict) * len(learning_rate_decay_dict)))
    ...
    # - Naive Bayes classifier [DOESN'T WORK]
